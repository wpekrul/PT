...
class AboutRegularExpressions < Neo::Koan
  ...
  def test_question_mark_means_optional
    # when the line below is uncommented, i don't get a syntax error:
    #assert_equal ab, "dfsd"[//]
  ...
  def test_character_classes_can_include_ranges
    assert_equal "42", "the number is 42"[/[0-9]+/] # argh. perl is smart about this, why can't ruby be?
   end
  ...
  def test_parentheses_also_capture_matched_content_by_number
    ...
    assert_equal "James", "Gray, James"[/(\w+), (\w+)/, 2] # this is an interesting syntax
  end
  ...
  # need to spend some more time on these next time through
